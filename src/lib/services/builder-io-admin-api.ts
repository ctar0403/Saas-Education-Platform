...
export class BuilderIOAdminApiService {
  private readonly apiKey: string;
  private readonly adminApiUrl = 'https://cdn.builder.io/api/v2/admin';
  private readonly contentApiUrl = 'https://builder.io/api/v1';
  ...
  private async makeGraphQLRequest<T>(
    ...
    try {
      console.log(`🔗 Builder.io Admin API GraphQL Request:`, { query: query.substring(0, 100) + '...', variables });
      ...
      if (!response.ok) {
        ...
        console.error(`❌ Builder.io Admin API Error (${response.status}):`, errorText);
        return {
          success: false,
      ...
      console.log(`✅ Builder.io Admin API Response:`, result);
      ...
      console.error('❌ Builder.io Admin API Request failed:', error);
      return {
        success: false,
  ...
  private async makeRestRequest<T>(
    ...
    try {
      const url = `${this.contentApiUrl}${endpoint}`;
      console.log(`🔗 Builder.io Content API Request: ${options.method || 'GET'} ${url}`);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`❌ Builder.io Content API Error (${response.status}):`, errorText);
        return {
          success: false,
          error: `HTTP ${response.status}: ${errorText}`,
        };
      ...
      console.log(`✅ Builder.io Content API Response:`, data);
      ...
      console.error('❌ Builder.io Content API Request failed:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      };
  ...
  /**
   * Get organization information and spaces using REST API
   */
  async getOrganization(): Promise<{ success: boolean; organization?: any; error?: string }> {
    const result = await this.makeRestRequest<any>('/me');

    if (result.success && result.data) {
      return {
        success: true,
        organization: {
          id: result.data.organizationId || 'current-org',
          name: result.data.organizationName || 'Current Organization',
    ...
    return {
      success: false,
      error: result.error || 'Failed to get organization information',
    };
  }

  /**
   * Create a new Builder.io space using REST API
   */
  async createSpace(request: CreateSpaceRequest): Promise<{ success: boolean; space?: BuilderSpace; error?: string }> {
    ...
    const spaceData = {
      name: request.name,
      type: request.type || 'Fusion'
    };

    const result = await this.makeRestRequest<BuilderSpace>('/spaces', {
      method: 'POST',
  ...
  async getSpaces(): Promise<{ success: boolean; spaces?: BuilderSpace[]; error?: string }> {
    ...
    if (orgResult.success && orgResult.organization?.spaces) {
      return {
        success: true,
        spaces: orgResult.organization.spaces,
      };
    }

    // Fallback: try to get spaces directly
    const result = await this.makeRestRequest<{ results: BuilderSpace[] }>('/spaces');
    if (result.success && result.data) {
      return {
        success: true,
    ...
    return {
      success: false,
      error: orgResult.error || result.error || 'Failed to fetch spaces',
    };
  }

  /**
   * Create a new page in a specific space using Content API
   */
  async createPage(
    spaceId: string,
    ...
    const result = await this.makeRestRequest<BuilderPage>(`/write/${spaceId}`, {
      method: 'POST',
  ...
  async getPages(spaceId: string, modelId: string = 'page'): Promise<{ success: boolean; pages?: BuilderPage[]; error?: string }> {
    ...
    const result = await this.makeRestRequest<{ results: BuilderPage[] }>(`/content/${spaceId}/${modelId}`);
  ...
  async deletePage(spaceId: string, pageId: string): Promise<{ success: boolean; error?: string }> {
    ...
    const result = await this.makeRestRequest(`/write/${spaceId}`, {
      method: 'DELETE',
          ...
          children: [
            ...
            },
            {
              ...
              component: {
                name: 'Text',
                options: {
                  text: 'Join thousands of satisfied customers who have achieved their goals',
                  style: {
                    fontSize: '1.25rem',
                    marginBottom: '2rem',
                    opacity: 0.9,
...
export default BuilderIOAdminApiService;
