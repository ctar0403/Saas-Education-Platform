...
const GeneralProducts: React.FC<GeneralProductsProps> = ({ 
  className = "", 
  apiEndpoint 
}) => {
  ...
  const fetchProducts = async (page: number) => {
    ...
    try {
      ...
      setError("Failed to load general products. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleProductClick = (product: Product) => {
    console.log("General product clicked:", product);
    ...
    <div className={`py-12 px-8 ${className}`}>
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600">No general products available.</p>
          </div>
        )}
      </div>
    </div>
