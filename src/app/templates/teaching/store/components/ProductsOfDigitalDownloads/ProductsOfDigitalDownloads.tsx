...
interface ProductsOfDigitalDownloadsProps {
  className?: string;
  apiEndpoint?: string;
}

const ProductsOfDigitalDownloads: React.FC<ProductsOfDigitalDownloadsProps> = ({ 
  className = "", 
  apiEndpoint 
}) => {
  ...
  const fetchProducts = async (page: number) => {
    ...
    try {
      ...
      setError("Failed to load digital download products. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleProductClick = (product: Product) => {
    console.log("Digital download product clicked:", product);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mt-2">Loading digital download products...</p>
          </div>
        ) : products.length > 0 ? (
          <>
          ...
          <div className="text-center py-12">
            <p className="text-gray-600">No digital download products available.</p>
          </div>
        )}
      </div>
    </div>
...
export default ProductsOfDigitalDownloads;
