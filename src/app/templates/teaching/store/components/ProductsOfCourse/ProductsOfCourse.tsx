...
interface ProductsOfCourseProps {
  className?: string;
  apiEndpoint?: string;
}

const ProductsOfCourse: React.FC<ProductsOfCourseProps> = ({ 
  className = "", 
  apiEndpoint 
}) => {
  ...
  const fetchProducts = async (page: number) => {
    ...
    try {
      ...
      // Fallback to empty state - the API service should handle fallbacks
      setProducts([]);
      setPaginationInfo({
        total_items: 0,
        next_cursor: null,
    ...
    <div className={`py-12 px-8 ${className}`}>
      <div className="max-w-7xl mx-auto">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              🎭 <strong>Development Mode:</strong> Using mock data. Configure <code>NEXT_PUBLIC_API_URL</code> in .env.local to connect to your API.
            </p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600">No course products available.</p>
          </div>
        )}
      </div>
    </div>
